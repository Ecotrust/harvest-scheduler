...
int read_in_adj_file(char* fname)
{
    ...
    while (fgets( col_header, MAX_STR_LENGTH, fin))
    {
        ...
        for ( i = 0; i < 2; i++ )
        {
            ...
            {
            case 0:
                new_poly = atoi( curr_field );
                break;
            case 1:
                adj_list[i] = atoi( curr_field );
                break;
            }
    ...
    while (fgets( col_header, MAX_STR_LENGTH, fin))
    {
        ...
        for ( i = 0; i < 5; i++ )
        {
            ...
            case 0:
                ...
                break;
            case 1:
                new_acres = atof( curr_field );
                break;
            case 2:
                strncpy( new_ownership, curr_field, strcspn( curr_field, keys ));
                new_ownership[strcspn( curr_field, keys )] = '\0';
                break;
            case 3:
                strncpy( new_strata, curr_field, strcspn( curr_field, keys ));
                new_strata[strcspn( curr_field, keys )] = '\0';
                break;
            case 4:
                new_rx = atoi( curr_field );
                break;
            }
            curr_field = strtok( NULL, ",");
        }
    ...
    while (fgets( col_header, MAX_STR_LENGTH, fin))
    {
        ...
        for ( i = 0; i < 6; i++ )
        {
            ...
            case 2:
                ...
                break;
            case 3:
                new_tp = atoi( curr_field );
                break;
            case 4:
                strncpy( new_species, curr_field, strcspn( curr_field, keys ));
                new_species[strcspn( curr_field, keys )] = '\0';
                break;
            case 5:
                new_cutvol = atof( curr_field );
                break;
            }
            curr_field = strtok( NULL, ",");
        }
...
skipcount:
    ...
    {
        volume_total = 0.0;
        sprintf(gs, "Init Cut Volume Per Period - Default Perscription and offset %d::",index1);
        logit(1,gs);
        for (tp=0; tp<=NTP; tp++)
        {
...
skip_adjcheck:
    ...
    sprintf(gs, "----------------------------------------------");
    logit(1,gs);
    sprintf(gs, "Ending Cut Volume Per Period - Offsets Applied::   ");
    logit(1,gs);
    volume_total = 0.0;
    ...
    sprintf(gs, "----------------------------------------------");
    logit(1,gs);
    sprintf(gs, "Total Volume for %s = %7.4f",ownership,volume_total);
    logit(1,gs);
    sprintf(gs, "Final objective function for %s = %7.4f",ownership,obj);
    logit(2,gs);
    sprintf(gs, "Best objective function seen for %s = %7.4f",ownership,best_obj);
    logit(2,gs);
    sprintf(gs, "----------------------------------------------\n");
    logit(1,gs);
